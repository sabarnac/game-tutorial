#ifndef MODELS_START_MODEL_CPP
#define MODELS_START_MODEL_CPP
...
/**
 * Class that represents a start button model.
 */
class StartModel : public ModelBase<StartModel>
{
private:
...
public:
  StartModel(const std::string &modelId)
      : ModelBase(
            modelId,
   ...
   */
  const static std::shared_ptr<StartModel> create(const std::string &modelId)
  {
    return std::make_shared<StartModel>(modelId);
  }
