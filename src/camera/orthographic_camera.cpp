#ifndef MODELS_ORTHOGRAPHIC_CAMERA_MODEL_CPP
#define MODELS_ORTHOGRAPHIC_CAMERA_MODEL_CPP
...
/**
 * Class that represents a 2D orthographic-based camera.
 */
...
public:
  OrthographicCamera(const std::string &cameraId)
      : CameraBase(
            cameraId,
            "Orthographic",
            glm::vec3(0.0f),
            ...
            glm::ortho(-1.0f / ASPECT_RATIO, 1.0f / ASPECT_RATIO, -1.0f, 1.0f, 0.1f, 100.0f)),
  ...
  /**
   * Creates a new instance of the orthographic camera.
   */
  const static std::shared_ptr<OrthographicCamera> create(const std::string &cameraId)
  {
    return std::make_shared<OrthographicCamera>(cameraId);
  }
  ...
  {
    return glm::ortho(-1.0f / aspectRatio, 1.0f / aspectRatio, -1.0f, 1.0f, nearPlane, farPlane);
  }
};

// Initialize the mouse speed static variable.
const float_t OrthographicCamera::mouseSpeed = 5.0f;
// Initialize the keyboard speed static variable.
const float_t OrthographicCamera::keyboardSpeed = 20.0f;
#endif